###################################################################################
# V 0.5 monitorizar (Red Hat Linux)                                               #
# Server: eadmin.dipcas.es IP: 195.57.195.23                                      #
#                                                                                 #
...
clear;
echo "***************************************************************************";
echo "*      MONITORITZACIO DE SERVEIS I LOGS DE EADMIN.DIPCAS.ES            *";
echo "***************************************************************************";
echo "";
echo "Tria una opcio:";
echo "------------------------------";
...
read OPCIO
 ...
 case $OPCIO in
   ...
   b|B)
        export GREP_OPTIONS='--color=auto' GREP_COLOR='104;8'
        if ps -ef | grep 'xaloc/servidores/apache-tomcat-5.5.27'; then echo ""; fi
        echo ""
        echo "Pulsa ENTER per a tornar"
        read
        monitorizar;;
   ...
   c|C)
        ...
        monitorizar;;
   c1|C1)
        sh /opt/xaloc/servidores/tomcat_sipac_sitae/parada.sh &
	...
	echo "Pulsa ENTER per a tornar"		
        ...
        monitorizar;;
   c2|C2)
        sh /opt/xaloc/servidores/tomcat_sipac_sitae/arranque.sh &
